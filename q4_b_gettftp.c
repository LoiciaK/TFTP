...
int main(int argc, char *argv[]){
    ...
    myaddr->sin_port = htons(1069); // conversion 
    inet_aton("127.0.0.1", &myaddr->sin_addr);

    requete(argv[2], "octet", sfd, res); //on choisit le mode de transmission en octets



    int lenDATA = 516; //512 octets est la taille max en mode octets + 4 octets pour coder 03 et Block# du paquet DATA
    char* DAT=(char *)malloc(lenDATA*sizeof(char));
    
    //struct sockaddr_storage addr;
    //socklen_t lensock = sizeof(struct sockaddr);
	...
	write(f, DAT+4,lenDATA);
	ssize_t nbr_ack = sendto(sfd, ACK, lenACK, 0, res->ai_addr, res->ai_addrlen);
	if(nbr_ack == -1){
		perror("sendto");
		exit(EXIT_FAILURE);
	} 
